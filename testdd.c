        ...
        while (h <= HEIGHT && i <= WIDTH)
        {
            ...
            else if (h <= ((HEIGHT - ray->hit->wallhei) / 2) + ray->hit->wallhei)
            {
                    if (checkmaphawall(ray,ray->hit->wallhitx,ray->hit->wallhity,50) == 1)
                     my_mlx_pixel_put(ray,i,h,colors_img(ray, (HEIGHT_TEXT * ((int)ray->hit->wallhity % 50)) / (int)ray->hit->wallhei, (HEIGHT_TEXT * 50) / (int)ray->hit->wallhei));
                     
//  (((m->dda->h - ((m->height - m->dda->wall) / 2)) * 64) / m->dda->wall), m->dda->direction));                       // my_mlx_pixel_put(ray,i,h,0xEF1DF2);
                    if (checkmaphawall(ray,ray->hit->wallhitx,ray->hit->wallhity + 1,50) == 1)
                    //  my_mlx_pixel_put(ray,i,h,colors_img(ray, (int)ray->hit->wallhitx % 50,(int)ray->hit->wallhity % 50));
                        my_mlx_pixel_put(ray,i,h,0xFFB833);
                    if (checkmaphawall(ray,ray->hit->wallhitx + 1,ray->hit->wallhitx + 1,50) == 1)
                    {
                    //  my_mlx_pixel_put(ray,i,h,colors_img(ray, (int)ray->hit->wallhitx % 50,(int)ray->hit->wallhity % 50));
